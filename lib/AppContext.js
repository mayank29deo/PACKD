...
export function AppProvider({ children }) {
  ...
  // Helper: hydrate local (non-profile) state from localStorage
  ...
  useEffect(() => {
    ...
    if (sessionStatus === 'authenticated' && session?.user?.email) {
      // ── Logged in: load profile + activities from Supabase ──────────────
      Promise.all([
        ...
        // Still load local-only state (RSVPs, swipes, DMs etc.) from localStorage
        if (stored) hydrateLocal(stored);
        setHydrated(true);
      });
    } else {
  ...
  const toggleRsvp = useCallback((eventId) => {
    setRsvps((prev) => {
      ...
      setEvents((evs) =>
        evs.map((e) =>
          e.id === eventId
            ? { ...e, rsvp: next[eventId] ? e.rsvp + 1 : Math.max(0, e.rsvp - 1) }
            : e
        )
      );
      if (next[eventId]) {
  ...
  const toggleJoinPack = useCallback((packId) => {
    setJoinedPacks((prev) => {
      ...
      setPacks((ps) =>
        ps.map((p) =>
          p.id === packId
            ? { ...p, members: next[packId] ? p.members + 1 : Math.max(0, p.members - 1) }
            : p
        )
      );
      if (next[packId]) {
